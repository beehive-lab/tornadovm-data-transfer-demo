...
public class Scenario1_FitsInGPU {
    ...
    public static void run(int size) throws TornadoExecutionPlanException {
        ...
        TaskGraph tg = new TaskGraph("saxpy-s1")
        ...
        try (TornadoExecutionPlan executionPlan = new TornadoExecutionPlan(tg.snapshot())) {
            ...
            for (int i = 0; i < iterations; i++) {
                SaxpyKernel.saxpy2(xJava, yJava, alpha);
            }

            // Validate results
            boolean status = Validator.saxpy(yJava, y);
            System.out.printf("[Scenario 1 - Transfer output in every execution] Size=%d, Iterations=%d, Total=%.2f ms, Validation=%s%n", size, iterations, ms, Validator.isValid(status));
        }
    }
}
